  ...
  const handleSubmit = async () => {
    ...
    try {
      const customerPayload = {
        ...
        shippingAddress1: sameAsBilling ? formData.billingAddress1 : formData.shippingAddress1,
        shippingAddress2: sameAsBilling ? formData.billingAddress2 : formData.shippingAddress2,
      ...
      const credentialsResult = await createCustomerCredentials(customerPayload);

      if (!credentialsResult.success) {
        alert(`Credentials creation failed: ${credentialsResult.error}.`);
        setIsSubmitting(false);
        return;
      }
      ...
      alert('Your data is successfully registered. Please wait for the approval to login and process further.');
      navigate('/login');

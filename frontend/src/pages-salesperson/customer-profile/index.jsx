...
const SPCustomerProfile = () => {
  const navigate = useNavigate();
  const [error, setError] = useState(null);


  const salespersonName = localStorage.getItem('salespersonName') || 'N/A';
  const salespersonCode = localStorage.getItem('salespersonCode') || 'N/A';
  const level = localStorage.getItem('level') || 'N/A';
  // const credentials = localStorage.getItem('credentials') || '';
  const email = localStorage.getItem('salespersonEmail') || 'N/A';
  const phone = localStorage.getItem('salespersonPhone') || 'N/A';
  ...
  return (
    <>
      <Helmet>
        <title>Salesperson Profile - Veeba Foods</title>
        <meta name="description" content="View your salesperson profile on Veeba Foods portal" />
      </Helmet>
            ...
            <div className="bg-gradient-to-r from-primary/10 to-accent/10 rounded-xl p-6 md:p-8 mb-6 md:mb-8 border border-primary/20">
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
                <div className="w-20 h-20 bg-primary/20 rounded-xl flex items-center justify-center flex-shrink-0">
                  <Icon name="UserCircle" size={48} color="var(--color-primary)" />
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-warning/10 text-warning rounded-md text-sm font-caption font-medium">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {profileFields.map((field) => (
                    <div key={field.label} className="space-y-1.5">
                      ...
                      <div className={`
                        flex items-center gap-3 px-4 py-3 rounded-lg border border-border bg-muted/30
                      `}>
                        <Icon
                          name={field.icon}
                          size={18}
                          color={field.highlight ? 'var(--color-primary)' : 'var(--color-muted-foreground)'}
                        />
                        <span className={`
                          font-body text-sm
                          ${field.highlight ? 'text-primary font-semibold' : 'text-foreground'}
                        `}>
                          {field.value}
                        </span>
                      </div>
...
export default SPCustomerProfile;